...
export const UserPhotoItem: React.FC<props> = ({ user, publicPhoto }) => {
  ...
  return (
    <li key={publicPhoto.id} className='mb-10 flex flex-col'>
      ...
      <div>
        <Link href={`/${user.id}/photo/${publicPhoto.id}`}>
      ...
      <div>
        コメント数: <Link href={`/${user.id}/photo/${publicPhoto.id}#comments`}><a className='px-1 underline'>{publicPhoto.comments?.length ?? 0}</a></Link>件
      </div>
      {sessionUser?.id === user.id && (
        <div className='flex flex-col pt-2'>
          ...
          <div className='flex'>
            <div>
              <button onClick={() => Router.push(`/${user.id}/photo/${publicPhoto.id}/edit`)} className='border-gray-300 border-2 rounded p-1 w-12 mr-2'>編集</button>
            </div>
            <div>
